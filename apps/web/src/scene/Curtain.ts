...
export class Curtain {
  ...
  async sweep(swap: () => void): Promise<void> {
    // ★ Друга завіса ЧЕКАЄ на першу, а не зникає.
    //
    // Доти вона просто виходила («якщо зайнято — нічого не роблю»), і разом із нею тихо зникав
    // `swap` — тобто те, заради чого її кликали. На живому шляху це виглядало так: гість відкрив
    // Дошку (завіса), одразу кинув тему — і локація не відкривалась НІКОЛИ. Розмова лишалась на
    // мапі, а люди, яких код перед тим поставив на місце віча, читались як «телепортувались».
    while (this.busy) await wait(80);
    this.busy = true;
