...
// Оригінальний шейдер води (як тік раніше): зсув поверхні за flow-мапою + легкі брижі.
// Єдина зміна проти оригіналу — маска домножена на «синє» (bluish), щоб ефект не
// розтікався на сушу, якщо flow-мапа трохи не збігається із запеченою рікою.
const waterFrag = `
...
void main(){ vec2 uv=vTextureCoord; vec4 fl=texture2D(flowTex,uv);
  ...
  vec3 col=mix(base.rgb, water, clamp(wmask*1.2,0.,1.)); col+=streak*0.4*vec3(0.10,0.14,0.17);
