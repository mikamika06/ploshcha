...
class ToolResult(BaseModel):
    """`found` відрізняє «інструмент відпрацював, але даних немає» від «інструмент зламався».

    Досі це жило лише всередині payload під ключем «відомо»/«known», тобто кожен споживач мусив
    угадати домовленість. Відсутність даних — це ДАНІ, і вона мусить бути в контракті порту:
    на ній стоїть і чесна відмова, і рішення верифікатора, чи є під відповіддю доказ.
    `None` = питання незастосовне (інструмент не шукає, а обчислює).
    """
