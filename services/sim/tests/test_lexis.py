...
def test_auto_derivation_accepts_every_attested_sense():
    """Правило змінено ПІСЛЯ заміру, і це оголошено: набір міряє ЗНАННЯ СЛОВА, а не вибір значення.

    Спершу ключі бралися лише з першого значення — щоб «банити» (мити / лаяти) не приймало «лаяти»
    на прикладі про мити голову. Але ламається симетрично: у «банітувати» приклад Панаса Мирного
    ілюструє ДРУГЕ значення, і правильна відповідь читалась як провал. Механічно визначити, яке
    значення ілюструє приклад, неможливо.
    """
    import sys
    ...
    keys, _ = derive_keys("банітувати", ["заст. засуджувати кого-небудь на вигнання",
                                         "діал. дуже сильно лаяти кого-небудь"])
    assert "засуджувати" in keys and "лаяти" in keys
