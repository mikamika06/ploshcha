...
# Розширення набору (U5-SCALE). Слова взяті з категорії «Діалектні вирази/uk» механічно (перші, що
# проходять правило придатності), а страт визначає НЕ моя оцінка, а ПОМІТКА ДЖЕРЕЛА: є `діал.`/
# `заст.`/`зах.`/`рідк.`/`етн.` → `rare`, немає → `common` (контроль). Правило строгіше за авторський
# поділ перших 24 слів, тому злиття не може розбавити рідкісний страт легкими словами.
POOL = [
    "адамашка", "адіть", "ажень", "антипко", "анцибол", "арідник", "бабка", "бабкуватий",
    "баблятися", "бабця", "бавити", "бавитися", "багаття", "багнити", "багнути", "багнутися",
    "багнітувати", "бадіка", "байда", "байдак", "бакаюватий", "бакша", "балабайка", "балабон",
    "балабушка", "балаканка", "баламкати", "баламут", "балювати", "балюватися", "банда", "банити",
    "бануш", "банітувати", "бараболька", "бараболя", "барани", "барда", "баркан", "бархатин",
]

POOL2 = [
    "аблегат", "авжеж", "альфатер", "анциболот", "анциболотник", "багачик", "балабончик",
    "балабуха", "баламкання", "банник", "банька", "бархатинний", "баскаличитися", "батова",
    "батура", "бахмат", "бахур", "бахурня", "бачний", "бачно", "бачність", "баштан", "бганок",
    "бгати", "бджоляник", "бебех", "бевка", "бегетати", "безбач", "безвиглядний", "безвстидний",
    "безвстидник", "безвстидниця", "безвстидність", "безденний", "бездужний", "бездух",
    "безділля", "безздоров'я", "безздорівний", "безличний", "безличник", "безличниця",
    "безлично", "безличність", "безмаль", "безмізкий", "безнапасний", "безневинне",
    "безодголосний", "безодголосно", "безодній", "безпальчий", "безперемінно", "безпереч",
    "безприхильник", "безпричальний", "безпричасний", "безпричасно", "безпровиння", "безрадний",
    "безрадно", "безрадність", "безсилок", "безслихи", "безум", "безчельний", "безчельник",
    "бейкатися", "белендіти", "бербениця", "бервення", "бервено", "берегиня", "береговина",
    "берегівка", "березка", "бересклет", "бересклетовий", "бесіда", "бесідливий", "бештанина",
    "бештати", "билень", "бинда", "биндовий", "биндочка", "бирка", "бисаги", "бистрень",
]

MARKS = ("діал.", "заст.", "зах.", "рідк.", "етн.", "гуцул", "лайл.")
...
def _stratum_by_label(entry: dict) -> str:
    """Страт нових слів визначає джерело, не я: помітка `діал.`/`заст.`/… → rare, інакше common.

    Дивимось на ПЕРШЕ значення, бо приклад слововжитку ілюструє саме його: помітка в третьому
    значенні не робить рідкісним те, що вжите в реченні.
    """
    first = entry["значення"][0].casefold() if entry["значення"] else ""
    return "rare" if any(m in first for m in MARKS) else "common"
...
def main() -> None:
    ...
    entries, rejected = [], []
    seen: set[str] = set()
    for declared, words in (("rare", RARE), ("common", COMMON), ("absent", ABSENT), (None, POOL), (None, POOL2)):
        for word in words:
            if word in seen:
                continue
            seen.add(word)
            ...
            label = declared or "?"
            if entry is None:
                rejected.append({"слово": word, "страт": label, "причина": "немає статті"})
                continue
            if reason := _usable(entry):
                rejected.append({"слово": word, "страт": label, "причина": reason})
                continue
            stratum = declared or _stratum_by_label(entry)
            entries.append(entry | {"страт": stratum,
                                    "поділ": "авторський" if declared else "за поміткою"})
